...
fn runStepNames(
    ...
    const err_output_file = try std.fs.cwd().createFile("last_compile_errors.txt", .{
        .truncate = true,
    });
    {
        ...
        for (0..steps_slice.len) |i| {
            ...
            thread_pool.spawnWg(&wait_group, workerMakeOneStep, .{
                &wait_group, b, step, step_prog, run, err_output_file,
            });
        }
    }
...
fn workerMakeOneStep(
    ...
    if (show_error_msgs or show_compile_errors or show_stderr) {
        ...
        var errwriter = err_file.writer(&writer_buf);
        var errout = &errwriter.interface;
        printErrorMessages(run.gpa, s, .{ .ttyconf = .no_color }, errout, run.prominent_compile_errors) catch {};
        errout.flush() catch {};
    }
    ...
    handle_result: {
        ...
        for (s.dependants.items) |dep| {
            thread_pool.spawnWg(wg, workerMakeOneStep, .{
                wg, b, dep, prog_node, run, err_file,
            });
        }
    }
